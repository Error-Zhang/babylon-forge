import { WebGPUEngine, Scene, type EngineCapabilities } from '@babylonjs/core';
...
export class BabylonPerformanceMonitor {
	...
	constructor(engine: WebGPUEngine, scene: Scene, config: BabylonMonitorConfig = {}) {
		this.engine = engine;
		this.scene = scene;
	...
	public start(): void {
		...
		console.log('Babylon performance monitoring started');
	}
	 ...
	 * 更新历史数据
	 */
	private updateHistoryData(fps: number, vertices: number): void {
 ...
 * Babylon.js 完整性能监控面板
 */
export class BabylonPerformancePanel {
	private monitor: BabylonPerformanceMonitor;
	...
	private eventHandlers = {
		...
		minimizeHandler: () => this.toggleMinimize(),
	...
	constructor(monitor: BabylonPerformanceMonitor, config: BabylonPanelConfig = {}) {
	 ...
	 * 清理曲线选择器的事件监听器
	 */
	private cleanupCurveSelectorListeners(): void {
	 ...
	 * 应用样式
	 */
	private applyStyles(panel: HTMLDivElement): void {
	 ...
	 * 设置曲线选择器事件
	 */
	private setupCurveSelector(): void {
		const checkboxes = this.panelElement.querySelectorAll('.curve-checkbox input[type="checkbox"]');
		checkboxes.forEach((checkbox: Element) => {
			const inputCheckbox = checkbox as HTMLInputElement;
			const curveId = inputCheckbox.value;
	 ...
	 * 设置拖拽功能
	 */
	private setupDragFunctionality(header: HTMLElement): void {
	 ...
	 * 更新指标显示
	 */
	private updateMetricsDisplay(metrics: BabylonPerformanceMetrics): void {
		// 基础性能
	 ...
	 * 更新WebGPU信息
	 */
	private updateWebGPUInfo(metrics: BabylonPerformanceMetrics): void {
	 ...
	 * 更新图表
	 */
	private updateGraph(metrics: BabylonPerformanceMetrics): void {
	 ...
	 * 绘制曲线
	 */
	private drawCurve(data: number[], color: string, minVal: number, maxVal: number, offsetY: number): void {
	 ...
	 */
	private drawBabylonGraph(historyData: any): void {
		if (!this.canvas || !this.ctx) return;
	 ...
	 * 更新状态指示器
	 */
	private updateStatusIndicator(metrics: BabylonPerformanceMetrics): void {
