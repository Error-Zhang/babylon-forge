...
type ActionType = 'moveForward' | 'moveBackward' | 'moveLeft' | 'moveRight' | 'jump' | 'fly' | 'sneak' | 'sprint' | 'attack' | 'break' | 'interact';
...
export class PlayerInputSystem {
	...
	constructor(
		...
		private readonly actionMap: ActionMap = DEFAULT_ACTION_MAP
	) {
	...
	onAction(action: ActionType, start: () => void, end: () => void) {
		this.onActionStart(action, start);
		this.onActionEnd(action, end);
	}
	...
	private initEventListeners() {
		this.scene.onKeyboardObservable.add((kbInfo) => {
			...
			const isDown = type === KeyboardEventTypes.KEYDOWN;
			this.handleKey(event.code, isDown);
		});

		this.scene.onPointerObservable.add((pointerInfo) => {
			const { type, event } = pointerInfo;
			const isDown = type === PointerEventTypes.POINTERDOWN;
			this.handleKey(`Mouse${event.button}`, isDown);
		});
	}
	...
	private checkActionStates() {
		// 遍历所有动作配置
		(Object.entries(this.actionMap) as [ActionType, ActionMap[ActionType]][]).forEach(([action, config]) => {
	...
	private checkActionConfig(config: ActionMap[ActionType]): boolean {
		...
		// 检测组合键
		const comboActive = config.comboKeys ? config.comboKeys.every((k) => this.keyStates.get(k)) : true;
